 ...
 *
 * <p>Поведение при невалидной записи различается по реализациям:
 * <ul>
 ...
 *       обрабатываются дальше — чтобы из-за пары кривых строк не рушить весь флоу.</li>
 ...
 *       {@link LoadResult#rejectedWarnings()} в этом случае обычно остаётся пустым.</li>
 * </ul>
 */
public interface DataSource {
    ...
    /**
     * @param loaded           массив записей (автобусов) успешно загруженных
     * @param rejectedWarnings причины, по которым отдельные записи были отброшены
     *                         (пустой список, если отбрасывать было нечего)
     */
